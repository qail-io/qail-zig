...
- Remaining real feature gap: accepted `GSSENCRequest` still fails closed because the encrypted GSS stream wrapper itself is not implemented yet.
- Remaining ergonomics gap versus `qail.rs`: Zig now has the preflight half and the session-aware callback shape, but still relies on caller-supplied GSS token providers rather than shipping the built-in Linux Kerberos provider/session stack.
...
1. Enterprise auth polish (full GSSENC encrypted stream + keep the local TLS client fork minimal and compatible across Zig upgrades)
