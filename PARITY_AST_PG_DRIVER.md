...
qail-zig ref: `7650362`
qail.rs ref: `11fd8342`
...
1. AST action surface parity: `71/71 = 100.0%`
- Matched after normalizing equivalent names (`TxnStart -> begin`, `Export -> copy_out`, etc.)
- Extra in `qail-zig`: `raw`

2. AST-native encoder parity vs `qail.rs` AST encoder: `35/35 = 100.0%`
- Rust encoder action set (`pg/src/protocol/ast_encoder/mod.rs`): 35 actions
- Zig encoder action set now includes all Rust-covered actions, plus Zig-only transaction/raw actions.

3. AST-native encoder coverage vs full Zig action enum: `72/72 = 100.0%`
...
Estimated parity: `~99%`
Estimated gap: `<1-2% behind qail.rs`

## High-Impact Gap List (Priority)

1. Finish enterprise auth polish
...
2. Keep Zig 0.16 migration isolated through `src/compat/*`
- Already centralized for network/io/time/rand/process/network callsites; keep new work behind compat aliases.

## Recommended Next Execution Order

1. Enterprise auth polish (handshake-native TLS leaf-cert extraction for SCRAM+ once std TLS exposes peer cert DER)
2. Zig 0.16 std I/O migration pass (retain compat aliases and avoid leaking std API changes into driver callsites)
