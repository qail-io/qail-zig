...
- `GSSENCRequest` acceptance now routes into an encrypted GSS stream on Linux; the remaining gap is runtime validation depth and ongoing maintenance of the local GSS/TLS compatibility layers rather than missing transport support.
- Remaining ergonomics gap versus `qail.rs`: much narrower now; the main delta is maturity/validation depth, not a missing enterprise-auth feature family.
...
1. Enterprise auth validation/maintenance (exercise the Linux GSSENC path against real Kerberos environments and keep the local TLS client fork minimal across Zig upgrades)
