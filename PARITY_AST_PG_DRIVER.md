...
- Supported: basic driver, pooling, prepared cache, pipeline fast paths, COPY in/out, COPY helper variants (`copy_bulk`, `copy_bulk_raw`, `copy_export_raw`, `copy_export_stream_raw`), cancellation (module-level `cancel_query` + driver-level `get_cancel_key`/`cancel_token`/`cancel_query`), TLS, async connect, transaction/savepoint, startup auth (cleartext + MD5 + SCRAM-SHA-256 + SCRAM-SHA-256-PLUS policy selection), auth policy controls (allow/deny cleartext, MD5, SCRAM, Kerberos, GSSAPI, SSPI), LISTEN/NOTIFY (`listen/unlisten/unlisten_all/recv_notification/poll_notifications`), logical replication core (`connect_logical_replication`, `identify_system`, slot create/drop, `start_logical_replication`, `recv_replication_message`, `send_standby_status_update`), timeout-aware high-level connect APIs (`connect_with_timeout`, `connect_with_auth_timeout`, `connect_logical_replication_with_auth_timeout` and password/plain variants, including TLS connect-timeout path), URL/env/builder connection ergonomics (`connect_url`, `connect_env`, `connect_with_options`, `builder`), URL query auth/TLS mapping (`auth_*`, `auth_mode`, `channel_binding`, `connect_timeout`, `replication`, `sslmode`, `sslrootcert`, `tls_server_end_point_cert_der`, `gssencmode`), `PgDriver` transport unification (`sslmode=prefer|require` now routes through `TlsConnection` on `connect_with_options`), centralized RLS context SQL helpers (`RlsContext`, `context_to_sql*`, `set_rls_context*`, `clear_rls_context`) and pooled scoped APIs (`acquire_with_rls*`, `with_rls/with_system/with_global/with_tenant`, auto-reset-on-release), table-level RLS helpers (`enable_rls/disable_rls/force_rls/no_force_rls`), EXPLAIN estimate helpers (`explain_estimate` / `explain_estimate_sql`), enterprise auth handshake hooks (Kerberos/GSS/SSPI via token provider callbacks, plus `GssTokenProviderEx` with stable per-handshake `session_id`), built-in Linux Kerberos preflight + provider stack (`linuxKrb5Preflight`, `linuxKrb5TokenProvider`) loaded via runtime GSSAPI lookup on Linux, plain TCP io backend runtime policy + transport (`QAIL_PG_IO_BACKEND=auto|sync|io_uring`; Linux `io_uring` stream path implemented for read/write in `driver/io_backend.zig`), hostname-based connect resolution across plain/TLS/GSSENC-preface paths, libpq-style GSSENCRequest preface negotiation (including CVE-style extra-byte detection and `.prefer` fallback on reject/server-error), and SCRAM+ TLS channel-binding auto-derivation from the handshake leaf cert with configured DER fallback (`TlsConfig.tls_server_end_point_cert_der`) and TLS-startup precedence that treats connection-derived binding bytes as authoritative over caller-supplied overrides
- Newly closed in this slice: public raw-runtime execution is fail-closed by default, typed RLS helper builders are available through `ast.policy`, schema policy parsing normalizes common `pg_dump` wrappers (`NULLIF(current_setting(...))::type`, `COALESCE(current_setting(...), 'false'::text)`), policy diffs now treat those wrappers as equivalent typed predicates instead of emitting churn, handwritten Zig `CTEDef` regained the typed `recursive_query` / `source_table` shape already present in Rust and the generated AST, TLS startup now fails closed for `channel_binding=require` unless the connection itself has binding bytes available, libpq-style GSSENCRequest negotiation now flows into a real encrypted GSS transport on Linux instead of failing closed after acceptance, and Zig now has the Linux Kerberos preflight and provider stack that previously existed only on the Rust side.
- Validation depth improved in this slice: the repository now includes a dedicated Linux Kerberos/GSSENC smoke workflow (`.github/workflows/enterprise-auth.yml`) that provisions a local realm + PostgreSQL service principal and runs an AST-native query over `gssencmode=require`.
...
2. Keep enterprise auth verified and maintainable
...
- `GSSENCRequest` acceptance now routes into an encrypted GSS stream on Linux, and CI now exercises a real Kerberos-backed smoke path. The remaining gap is broader runtime depth and ongoing maintenance of the local GSS/TLS compatibility layers rather than missing transport support.
- Remaining ergonomics gap versus `qail.rs`: much narrower now; the main delta is maturity/coverage depth, not a missing enterprise-auth feature family.
...
1. Enterprise auth validation/maintenance (expand beyond the new Linux GSSENC smoke path and keep the local TLS client fork minimal across Zig upgrades)
