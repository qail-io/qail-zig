...
/// Derived SCRAM `tls-server-end-point` binding from the handshake leaf cert,
/// when capture is enabled in `Options`.
...
pub const Options = struct {
    ...
    write_buffer: []u8,
    read_buffer: []u8,
    /// When set, the handshake derives SCRAM `tls-server-end-point` bytes from
    /// the leaf certificate and stores them on the returned client.
    channel_binding_allocator: ?std.mem.Allocator = null,
                ...
                switch (handshake_type) {
                    ...
                    .certificate => cert: {
                        ...
                        while (!certs_decoder.eof()) {
                            ...
                            if (cert_index == 0) {
                                if (options.channel_binding_allocator) |allocator| {
                                    const cert_der = certd.buf[certd.idx..][0..cert_size];
                                    tls_server_end_point_binding = try channel_binding.deriveTlsServerEndPointBindingFromSignatureAlgorithm(
                                        allocator,
                                        cert_der,
                    ...
                    .finished => {
                        ...
                        return .{
                            ...
                            .tls_server_end_point_binding = tls_server_end_point_binding,
                        };
                    },
                    else => return error.TlsUnexpectedMessage,
                }
