...
pub const LockMode = enum {
    ...
    pub fn toSql(self: LockMode) []const u8 {
        return switch (self) {
            .update => "FOR UPDATE",
...
pub const OverridingKind = enum {
    ...
    pub fn toSql(self: OverridingKind) []const u8 {
        return switch (self) {
            .system_value => "OVERRIDING SYSTEM VALUE",
            .user_value => "OVERRIDING USER VALUE",
        };
    }
};
...
pub const SampleMethod = enum {
    ...
    pub fn toSql(self: SampleMethod) []const u8 {
        return switch (self) {
            .bernoulli => "BERNOULLI",
            .system => "SYSTEM",
        };
